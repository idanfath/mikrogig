...
const OnboardingProfile: InertiaPageWithLayout = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          const stateName = normWilayah(address.state || address.province || '');
          const cityName = normWilayah(
            address.city || address.regency || address.municipality || address.county || address.town || ''
          );
          ...
          const matchedProv = provinces.find((p) => {
            ...
            return name.includes(stateName) || stateName.includes(name);
          });

          if (!matchedProv) {
            throw new Error('Provinsi Anda tidak terdaftar di database.');
          }
          ...
          const regs = (await regResponse.json()) as Array<{ id: string; name: string }>;
          setRegencies(regs);
  ...
  const [lastEnhancedTitle, setLastEnhancedTitle] = useState<string | null>(null);
  ...
  const isProfileComplete = isFreelancer
    ...
    data.province_id !== '' &&
    data.regency_id !== ''
    : data.date_of_birth !== '' && data.province_id !== '' && data.regency_id !== '';
  ...
  return (
    ...
    >
        ...
        <FieldGroup>
              ...
              <Field>
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    disabled={!data.title.trim() || data.title === lastEnhancedTitle || enhancingTitle || processing}
                    onClick={() => handleEnhance('title', data.title)}
                  >
                    <Sparkles className={`size-3.5 text-violet-500 ${enhancingTitle ? 'animate-pulse' : ''}`} />
                    {enhancingTitle ? 'Memproses...' : 'Tingkatkan dengan AI'}
                  </Button>
                </div>
                <FieldDescription>
                  Contoh: Tukang Las, Cleaning Service,
                  Supir Pribadi, Buruh Bangunan, dll.
                </FieldDescription>
                <Input
                  id="title"
              ...
              <Field>
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="h-8 gap-1.5 text-xs"
                    disabled={!data.bio.trim() || data.bio === lastEnhancedBio || enhancingBio || processing}
                    onClick={() => handleEnhance('bio', data.bio)}
                  >
                    <Sparkles className={`size-3.5 text-violet-500 ${enhancingBio ? 'animate-pulse' : ''}`} />
                    {enhancingBio ? 'Memproses...' : 'Tingkatkan dengan AI'}
                  </Button>
                </div>
                <FieldDescription>
                  Ceritakan tentang dirimu dan
                  pengalamanmu
                </FieldDescription>
                <textarea
                  id="bio"
              ...
              <Field>
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="h-8 gap-1.5 text-xs"
                    disabled={!data.title.trim() || !data.bio.trim() || data.skills.length > 0 || enhancingSkills || processing}
                    onClick={handleEnhanceSkills}
                  >
                    <Sparkles className={`size-3.5 text-violet-500 ${enhancingSkills ? 'animate-pulse' : ''}`} />
          ...
          <Field orientation="horizontal">
            <Field>
              ...
              >
                <SelectTrigger id="province_id" className="w-full h-11">
            ...
            <Field>
              ...
              >
                <SelectTrigger id="regency_id" className="w-full h-11">
      ...
      <div className="flex w-full flex-col items-center gap-3 sm:items-end">
        ...
        <p className="text-center text-xs text-muted-foreground sm:text-right max-w-sm">
          Dengan mengklik "Simpan Profil", Anda menyatakan bahwa seluruh informasi yang Anda berikan telah dibaca dan dikonfirmasi kebenarannya.
        </p>
      </div>
    </form>
  );
