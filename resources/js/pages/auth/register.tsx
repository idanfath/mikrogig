...
const Register = () => {
  return (
    <Form action={register.submit.url()} method="post" className="w-full max-w-xl p-6">
      {({ errors, processing }) => (
        <>
          <Field className="mb-4" data-invalid={!!errors.name}>
            ...
            <FieldDescription>
              {errors.name && (
                <span className="text-destructive capitalize">{errors.name}</span>
              )}
            </FieldDescription>
          </Field>
...
Register.layout = (page: ReactNode) => (
  <AuthLayout
    ...
    footer={<p className="mt-4">Sudah punya akun? <a href={login.url()} className="font-semibold text-primary hover:underline">Masuk</a></p>}
  >
    {page}
  </AuthLayout>
);
