...
const floatingCardVariants = cva('rounded-2xl border', {
    variants: {
        variant: {
            ...
            glass: 'border-white/60 bg-white/80 shadow-soft backdrop-blur-xl',
            dark: 'border-white/10 bg-foreground text-background shadow-soft',
            orange: 'border-brand/20 bg-brand-soft text-foreground',
            ...
            {(icon || heading || value) && (
                <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3">
                        {icon && <div className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand-soft text-primary">{icon}</div>}
                        <div>
