    ...
    return (
        <Section id="how-it-works" className={cn(className)} {...props}>
            ...
            <div className="relative grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <div
                    className="absolute top-8 right-[12.5%] left-[12.5%] hidden border-t border-dashed border-brand/30 lg:block"
                    aria-hidden="true"
                />
                {steps.map((step, index) => (
                    ...
                    >
                        <div className="flex items-center justify-between gap-4">
                            <span className="grid size-13 place-items-center rounded-2xl border border-brand/20 bg-brand-soft text-primary [&_svg]:size-5">
                                {step.icon}
                            </span>
                        ...
                        {index < steps.length - 1 && (
                            <span
                                className="mt-6 block h-1 w-10 rounded-full bg-brand/25 lg:hidden"
                                aria-hidden="true"
                            />
                        )}
                    </Card>
