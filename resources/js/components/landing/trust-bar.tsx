...
const defaultItems: TrustItem[] = [
  ...
  },
  {
    ...
    label:
      'Dana diamankan sebelum mulai dan dilepas setelah pekerjaan selesai.',
    icon: <WalletCards />,
  },
];
    ...
    >
      <div className="mb-8 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="max-w-md text-sm leading-6 text-white/50">
          Bukan hanya tempat mencari kerja—setiap tahap dirancang untuk
      ...
      <div className="grid divide-y divide-white/10 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
        {items.map((item) => (
          ...
          >
            ...
            <p className="mt-2 text-xs leading-5 text-white/48">{item.label}</p>
          </div>
        ))}
      </div>
    </Section>
