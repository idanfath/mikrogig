...
const defaultTestimonials: TestimonialItem[] = [
  {
    quote:
      'Saya berpengalaman bekerja, tetapi belum pernah membuat CV dan kesulitan menulis profil panjang di ponsel.',
    name: 'Pekerja harian',
  ...
  {
    quote:
      'Tugas bertambah setelah saya datang, tetapi upah yang dijanjikan tetap sama dan kesepakatannya hanya lewat chat.',
  ...
  {
    quote:
      'Pekerjaan sudah selesai, tetapi pembayaran tertunda karena tidak ada bukti dan alur penyelesaian masalah yang jelas.',
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {items.map((item) => (
          <TestimonialCard
            key={item.name}
            name={item.name}
            role={[item.role, item.location].filter(Boolean).join(' · ')}
            message={item.quote}
            avatar={item.avatar}
            rating={item.rating}
