...
const featureCardVariants = cva('relative h-full overflow-hidden rounded-[1.5rem] border p-6 sm:p-7', {
    variants: {
        tone: {
            ...
            orange: 'border-primary/15 bg-accent/55',
        ...
        <div className={cn(featureCardVariants({ tone }), className)} {...props}>
            <div className="relative z-10 flex h-full flex-col">
                <div className="flex items-start justify-between gap-4">
                    <span className={cn('grid size-11 place-items-center rounded-xl', inverse ? 'bg-white/10 text-primary' : 'bg-white/80 text-primary shadow-sm')}>{icon}</span>
