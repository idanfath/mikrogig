...
final class DeclineGigAgreement
{
    ...
    public function execute(User $freelancer, Gig $gig): GigAgreement
    {
        ...
        [$agreement, $clientId] = DB::transaction(function () use ($persistedAgreement, $freelancerId, $freelancer, $gig): array {
            ...
            if ($lockedGig->status !== GigStatus::LockPending || $offer->status !== GigOfferStatus::ACCEPTED || $agreement->submitted_at === null) {
                throw new DomainException('Agreement cannot be declined in the current state.');
            }
            ...
            $agreement->closure_reason = GigAgreementClosureReason::FreelancerDeclined;
            $agreement->save();
        ...
        $this->realtime->stateChanged($gig, GigRealtimeChange::Agreement, [$freelancer->id, $clientId]);
        $this->realtime->discoveryChanged($gig, GigDiscoveryChange::Upsert);
        $this->notify($freelancer->id, $clientId, $gig->id, 'Freelancer menolak syarat', 'Freelancer menolak syarat gig dan gig dibuka kembali.');
